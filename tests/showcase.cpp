...
void print(const Record &r)
{
    ...
    for (int i = 0; i < r.row.values.size(); i++)
    {
        cout << r.row.values[i];

        if (i + 1 != r.row.values.size())
            cout << " | ";
    }
...
void print(const vector<Difference> &v)
{
    if (v.empty())
    {
        cout << "No Differences\n";
...
int main()
{
    filesystem::remove("data/Students.db");
    Catalog catalog;
