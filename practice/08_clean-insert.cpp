...
class Table{
    ...
    bool validateValue(const string& value,const ColMeta& col){
        if(col.type==INT) return isInt(value);
        else if(col.type==FLOAT) return isFloat(value);
        else if(col.type==BOOL) return isBool(value);
        else if(col.type==STRING) return isString(value,col.size);
        return false;
    }
...
public:
    ...
    bool insert(const Row& row){
        //row proper,get pk,insert timestamp,deleted,insert row <- use seek
        if(!validateRow(row)) return false;

        string pk=getPrimaryKey(row);
        if(history.count(pk)) return false; //duplicate
