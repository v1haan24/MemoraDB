    ...
    if (s.where && s.where->op == CompareOp::SIMILAR_TO) {
        ...
        if (!vh) {
            return ExecResult::Error("No vector table found for '" + s.tableName +
                                     "' -- has the embedding worker run yet?");
        }
        if (!embedder) {
            return ExecResult::Error(
                "SIMILAR TO needs the query text embedded, but no embedding provider is "
                "configured. MemoraDB embeds asynchronously via the queue + Python worker "
                "(embedMake / ml_worker.py) and has no synchronous text->vector call; "
