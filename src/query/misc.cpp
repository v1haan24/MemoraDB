...
size_t count(const std::vector<Record>& records){return records.size();}

std::vector<Record> sortRecords(const std::vector<Record>& records,const TableMeta& meta,int32_t column,bool ascending){
    ...
    std::sort(ans.begin(),ans.end(),
        [&](const Record& a,const Record& b){
            if(col.type==INT){
                int32_t x=std::stoi(a.row.values[column]);
                int32_t y=std::stoi(b.row.values[column]);
