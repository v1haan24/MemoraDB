...
SelectStmt Parser::parseSelect() {
    ...
    // The temporal clause (AS OF / BETWEEN / SNAPSHOT) and WHERE (including a
    // semantic SIMILAR TO condition) are independent and may appear in either
    // order, so semantic + temporal queries work either way:
    //   ... AS OF <date> WHERE col SIMILAR TO "text"
    //   ... WHERE col SIMILAR TO "text" AS OF <date>
