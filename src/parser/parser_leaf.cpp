...
DateLiteral Parser::parseDateLiteral() {
    ...
    int32_t maxDay = daysInMonth(date.year, date.month);
    ...
    if (check(TokenType::INTEGER_LITERAL)) {       
        ...
        if (match(TokenType::COLON)) {
            const Token& minuteTok = expect(TokenType::INTEGER_LITERAL,
                                            "for the minute in a timestamp (expected HH:MM[:SS[.mmm]])");
            ...
            if (match(TokenType::COLON)) {               
                if (check(TokenType::FLOAT_LITERAL)) {
                    const Token& secondTok = advance();
                    const std::string& raw = secondTok.value;
                    size_t dot = raw.find('.');
                    std::string intPart = raw.substr(0, dot);
                    std::string fracPart = raw.substr(dot + 1);
                    while (fracPart.size() < 3) fracPart += '0';
                    fracPart = fracPart.substr(0, 3); 

                    date.second = std::stoi(intPart);
                    date.millis = std::stoi(fracPart);

                    if (date.second < 0 || date.second > 59) {
                        throw ParseError("Invalid second " + std::to_string(date.second) +
                                              " in timestamp (must be between 0 and 59)",
                                          secondTok.line, secondTok.column);
                    }
                } else {
                    const Token& secondTok = expect(TokenType::INTEGER_LITERAL,
                                                    "for the second in a timestamp (expected HH:MM:SS[.mmm])");
