...
const char* code(term::Color c) {
    switch (c) {
        ...
        case term::WHITE: return "\033[37m"; case term::LIGHT_YELLOW: return "\033[38;5;229m";
    }
    return "";
}
}
...
void term::clear() {
    ...
    if (colorsEnabled()) std::cout << "\033[3J\033[2J\033[H";
#else
    if (colorsEnabled()) std::cout << "\033[3J\033[2J\033[H";
...
void term::printTable(const std::vector<std::string>& h,
    ...
    auto row = [&](const auto& r, bool header = false) { std::cout << '|'; for (size_t i = 0; i < w.size(); ++i) {
        const std::string v = i < r.size() ? r[i] : "";
        const std::string out = header ? paint(v, {BOLD, CYAN}) : v;
        std::cout << ' ' << out << std::string(w[i] - v.size(), ' ') << " |";
    } std::cout << '\n'; };
    border(); row(h, true); border(); for (const auto& r : rows) row(r); border();
}
