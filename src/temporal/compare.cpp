...
std::vector<Difference> Table::evolution(const std::string& pk,uint64_t t1,uint64_t t2){
    ...
    std::ptrdiff_t idx = (start != nullptr) ? start - hist.data() : 0;
    if(idx >= static_cast<std::ptrdiff_t>(hist.size()) || hist[idx].timestamp > t2) return ans;
    while(idx+1<static_cast<std::ptrdiff_t>(hist.size()) && hist[idx+1].timestamp<=t2){
