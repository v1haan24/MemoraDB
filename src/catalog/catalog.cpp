...
bool Catalog::createTable(TableMeta& table){
        ...
        table.metadataSize=
            sizeof(int32_t)+tns+sizeof(int32_t)+sizeof(int32_t)+
            table.columnCount*(cns+sizeof(DataType)+sizeof(int32_t)+sizeof(int32_t)+sizeof(uint8_t)+sizeof(uint8_t));
...
TableMeta Catalog::readMetadata(const std::string& fileName){
        ...
        for(uint32_t i=0;i<temp.columnCount;i++){
            ColMeta col;
            readColumn(file,col);
